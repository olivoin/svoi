...
class WooCommerce_Show_Attributes {
	...
	private function __construct() {
		...
		add_action( 'woocommerce_single_product_summary', array( $this, 'show_atts_on_product_page' ), 15);
						...
						if ( $attribute['is_taxonomy'] ) {
							...
							if ( ! empty( $terms ) ) {
								if ( ! is_wp_error( $terms ) ) {
									...
									if ( $hide_labels != 'yes' ) {
										$out_middle .= '<span class="attribute-label"><span class="attribute-label-text katalog-beer-params-title">' . sprintf( __( '%s', 'woocommerce-show-attributes' ), $tax_label ) . '</span>' . $colon . ' </span> ';
									}
